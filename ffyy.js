 ...
 */
    for (let user of userList) {
        taskall.push(await user.user_info('登录'));
        await wait(1); //延迟
        await $.wait(2000);
        taskall.push(await user.user_feed('签到'));
        await wait(20); //延迟
        await $.wait(2000);
...
class UserInfo {
    constructor(str) {
        ...
        //let ck = str.split('&')
        //this.data1 = ck[0]
        this.host = "feifanapi.feifan.art";
 ...
 */
        ...
        async user_info(name) { // 登录
        try {
            let options = {
                method: "Post",
                url: `${this.hostname}/app/user/login/byPassword`,
                headers: {
                    'Accept': '*/*',
                    'platform': '1',
                ...
                body: `{"mobile":"${this.ck[0]}","password":"${this.ck[1]}"}`,
            };
            //console.log(options);
            ...
            if (result.ecode == 0) {
                ...
                DoubleLog(`账号[${this.index}]  登陆成功！`);
                DoubleLog(`账号[${this.index}]  获取Token成功！: ${result.data.token}`);
            } else {
 ...
 * @returns {Promise<unknown>}
 */
    async user_feed(name) { // 签到
        try {
            let options = {
                method: "Post",
                url: `${this.hostname}/app/egg/feed`,
                headers: { 
                    'Host':this.host,
                    'Accept':'*/*',
            ...
            if (result.ecode == 0) {
                ...
                DoubleLog(`账号[${this.index}]  用户查询:失败 ❌ 了呢:${result.emessage}`);
                //console.log(result);
            }
        } catch (error) {
            console.log(error);

...
let UserCookie = ($.isNode() ? process.env.yxjy : $.getdata('yxjy')) || '';
let UserCookieArr = [];
const {log} = console;
let data = '';
let msg =``;
var timestamp = Math.round(new Date().getTime()).toString();
let host = 'yixuejiayuan7.taishangyixue899.com';
...
function signIn(timeout = 2000) {
    return new Promise((resolve) => {
        let url = {
            url: `${hostname}/index.php/api/order/lessondaka`,
            headers: {
                Host: host,

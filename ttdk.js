...
Const通知=1 		//0为关闭通知,1为打开通知,默认为1
Const{日志}=控制台；
让scriptVersionLatest=""；
让UserCookie=($.isode()？过程。env.ymgw: $.getData('ymgw'))||'';
让UserCookieArr=[];
让 数据='';
让 味精=``;
让isSign=假的；
...
等候GetVersion()；
DoubleLog('\n================ 共找到${UserCookieArr.长度}个账号================\n脚本执行✌北京时间(UTC+8)：${新的日期(新的日期().getTime()+新的日期().getTimezoneOffset()*60*1000+8*60*60*1000).toLocaleString()}\n================版本对比检查更新================`);
如果(scriptVersionLatest！=scriptVersion){
DoubleLog('\n当前版本：${scriptVersion}`)
...
为(让指数=0；index<UserCookieArr.长度；index++){
让NUM=索引+1
                DoubleLog(`\n================ 开始第 ${num} 个账号 ================`)
...
function getVersion(timeout = 3 * 1000) {
    return new Promise((resolve) => {
        let url = {
            url: `https://ghproxy.com/https://raw.githubusercontent.com/YourAhTzu/YourAhTzu/main/ymgw.js`,
        }
